...
for (i in seq_along(month.name)) {
  month_strings[i] <- str_c("Month ", i, ": ", month.name)
}
...
unique_cut <- diamonds %>% pull(cut) %>% levels()
lin_mod_results <- vector(mode = "list", length = length(unique_cut))
# for loop
for (i in seq_along(unique_cut)) {
  this_cut <- unique_cut[i]
